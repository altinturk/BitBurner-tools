...
function sell(ns, stock, numShares) {
    ...
    //ns.print(`Sold ${stock.sym} for ${format(profit)}`);
    ns.print(`SELL: ${stock.sym} LOT: ${format(numShares)} Profit: ${format(profit)}`);
    ...
    while (true) {
        ...
        if ((numShares * stocks[0].expRet * stocks[0].price * numCycles) > commission) {
            ...
            if (numShares > stocks[0].maxShares)
                numShares = stocks[0].maxShares;
            buy(ns, stocks[0], numShares);
        }

        await ns.sleep(3 * 1000 * numCycles + 200);
    }
}